...
// Helper to clean BibTeX strings (remove braces, unescape common LaTeX
// escapes like \& \% \_ \$ so the result is valid inside a double-quoted
// YAML string in the generated frontmatter).
...
function importBibtex() {
  ...
  parsed.forEach(entry => {
    ...
    // Validate cover image existence. This lab's citations.bib doesn't set a
    // per-paper cover, and there's no generic placeholder asset shipped in
    // this project (removed with the rest of the demo content), so leave
    // `cover` unset rather than pointing at a fallback image that may not
    // exist - the schema field is optional and the UI already guards on it.
    if (cover) {
    ...
    // Include the citation key so two entries that truncate to the same
    // year-author-title slug (e.g. companion papers with near-identical
    // titles) don't silently overwrite one another.
    const keySlug = slugify(entry.citationKey, { lower: true, strict: true });
    const filename = `${year}-${firstAuthor}-${titleSlug}-${keySlug}.md`;
